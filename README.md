...
- 正文编辑与预览：加粗、斜体、删除线、标题、列表、引用、代码块、链接和表格，支持撤销 / 重做。
- 编辑后显式保存或取消；切换笔记、关闭窗口前提醒未保存的修改，远程新回复不会覆盖当前编辑。
...
正文中带空格的路径请用引号或 Markdown 行内代码包裹，避免把后面的说明文字当成路径。文件链接复制其完整目标路径，不包含源码行号；不会自动打开或执行文件。

点击右上角铅笔编辑正文，选中文字后点击删除线等格式按钮，再保存。支持 `Ctrl+S` 保存。修改仅影响本机笔记，不会改动原 Codex 会话。编辑器离线可用；首次编辑时保留一份原始正文，不是完整版本历史。
